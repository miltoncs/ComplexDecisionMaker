...
public class Maneuver implements Comparable<Maneuver>
{
    ...
    {
        return "<" + repeat("-", (int) initialDelay)
                    + repeat("*", (int) maneuverDuration)
                    + repeat(".", (int) buffer)
                    + ">";
    }
    ...
    {
        int delay = Math.toIntExact(Math.round((initialDelay * 100) / timeToFailure));

        int duration = Math.toIntExact(Math.round((maneuverDuration * 100) / timeToFailure));
        if (duration == 0) duration = 1;

        int remaining = 100 - (delay + duration);

        return  "<"
                + String.join("", Collections.nCopies(delay, "-"))
                + String.join("", Collections.nCopies(duration, "*"))
                + String.join("", Collections.nCopies(remaining, "-"))
                + ">";
    }
    ...
    public Maneuver withDelay(double newDelay)
    {
        ...
        {
            throw new RuntimeException("New delay is less than old delay...");
        }
